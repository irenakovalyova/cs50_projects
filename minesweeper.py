...
class Sentence():
    ...
    def inferred_sentence(self, other):
        ...
        if self.cells.issubset(other.cells):
            ...
            new_sentence.count = other.count - self.count
        if other.cells.issubset(self.cells):
            new_sentence.cells = self.cells - other.cells
            new_sentence.count = self.count - other.count
...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        # Mark any additional cells as safes or mines
        additional_mines = set()
        additional_safes = set()
        for sentence in self.knowledge:
            for cell in sentence.known_mines():
                additional_mines.add(cell)

            for cell in sentence.known_safes():
                additional_safes.add(cell)

        for cell in additional_mines:
            self.mark_mine(cell)
        for cell in additional_safes:
            self.mark_safe(cell)

        # Add any additional knowledge if it can be inferred from existing knowledge

        additional_knowledge = []

        for sentence, other in itertools.combinations(self.knowledge, 2):
            inference = sentence.inferred_sentence(other)
            if inference is not None and inference not in self.knowledge:
                additional_knowledge.append(inference)

        self.knowledge.extend(additional_knowledge)
